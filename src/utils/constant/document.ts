...
export const GROUP_TO_EXTENSIONS: Record<string, string[]> = {
  ...
  EXCEL: [FileExtension.XLS, FileExtension.XLSX, FileExtension.CSV],
...
const HOVER_STYLES: Record<string, string> = {
  ...
  CSV: "hover:bg-green-200 hover:border-green-300",
...
> = {
  ...
  CSV: {
    label: "CSV",
    styling: {
      selectedClass: SELECTED_STYLES.CSV,
      unselectedClass: `${FILE_EXTENSION_COLORS.CSV} ${HOVER_STYLES.CSV}`,
      icon: XlsIcon,
      iconBg: "bg-green-600",
    },
    onClick: (setType) => setType("XLSX"),
